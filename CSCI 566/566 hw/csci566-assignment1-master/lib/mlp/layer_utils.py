...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training and self.keep_prob != 0:
            kept = (self.rng.random(np.prod(feat.shape)).reshape(feat.shape) < self.keep_prob)/self.keep_prob
            # kept = (self.rng.random(feat.shape[0]) < self.keep_prob)/self.keep_prob
            output = feat * kept
        else:
            output = feat
        #############################################################################
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        loss = np.sum(-np.log(logit[np.arange(feat.shape[0]), label]))
    ...
    def backward(self):
        ...
        #############################################################################
        dlogit = logit.copy()
        dlogit[np.arange(logit.shape[0]), label] -= 1
        if self.size_average:
