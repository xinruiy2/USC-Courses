...
class Module(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        #Flatten data
        # feat = self.flatten_layer.forward(feat)
        output = feat
...
class TestFCReLU(Module):
    def __init__(self, keep_prob=0, dtype=np.float32, seed=None):
        ...
        self.net = sequential(
            ########## TODO: ##########
            flatten(name="flat"),
            fc(input_dim = 20, output_dim = 10, name="param_name"),
            relu()
            ########### END ###########
        )
...
class TinyNet(Module):
    def __init__(self, keep_prob=0, dtype=np.float32, seed=None, hidden_unit=5000):
        """ Some comments """
        # self.flatten_layer = flatten()
        self.net = sequential(
            ########## TODO: ##########
            flatten(name="flat"),
            fc(3072, 5000, 1e-2, name = "fc1"),
            relu(name="relu1"),
            fc(5000, 10, 1e-2, name = "fc2")
            ########### END ###########
        )
